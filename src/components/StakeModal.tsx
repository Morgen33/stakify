...
const stakeTypes: { type: StakeType; label: string; icon: typeof Lock; tip: string; desc: string }[] = [
  {
    type: "soft",
    label: "Soft Stake",
    icon: Unlock,
...
const StakeModal = ({ poolName, apy, rewardToken, lockPeriodDays, platformFeePct, trigger }: StakeModalProps) => {
  ...
  // Calculate effective APY based on stake type and duration
  const getEffectiveApy = () => {
    if (stakeType === "hard") return apy * 1.2; // 20% bonus for hard stake
    if (stakeType === "flexible") {
      const multiplier = Math.min(customDays / lockPeriodDays, 2);
      return apy * (0.5 + multiplier * 0.5);
    }
    return apy * 0.8; // Soft stake = 80% of base APY
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-card border-border max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          {/* Amount */}
          <div>
            <p className="text-xs text-muted-foreground mb-2 font-display tracking-wider">AMOUNT</p>
            <Input
              type="number"
              placeholder="0.00"
              value={amount}
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={!agreed || parsedAmount <= 0}
            >
